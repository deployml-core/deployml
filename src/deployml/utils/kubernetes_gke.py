...
def check_gke_cluster_connection(
    cluster_name: str, zone: Optional[str] = None, region: Optional[str] = None
) -> bool:
    ...
    """
    try:
        result = run_tool("kubectl", ["cluster-info"], capture_output=True, text=True)
...
def generate_mlflow_manifests_gke(
    ...
    if not backend_store_uri:
        backend_store_uri = (
            "sqlite:////mlflow-artifacts/mlflow.db"
            if use_pvc
...
def deploy_to_gke(
    ...
    try:
        ...
        if not service_name:
            typer.echo(
                "   Could not read the service name from service.yaml; "
        ...
        while waited < max_wait:
            ...
            if result.returncode == 0 and external_ip and external_ip != "<none>":
                ...
                port_cmd = [
                    ...
                    service_name,
                    "-o",
                    f"jsonpath={port_query}",
                ] + ns
                port_result = run_tool(
                    port_cmd[0], port_cmd[1:], capture_output=True, text=True
                )
