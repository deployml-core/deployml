...
class DeploymentStack:
        ...
        try:
            ...
            for key, value in outputs.items():
                ...
                if "mlflow" in key.lower() and output_val.startswith("http"):
                    ...
                    "fastapi" in key.lower()
                    or "serving" in key.lower()
                    and output_val.startswith("http")
                ):
        ...
        try:
            ...
            for key, value in outputs.items():
                ...
                if "instance_connection_name" in key.lower():
                    ...
                    "postgresql_credentials" in key.lower()
                    or "db_password" in key.lower()
                ):
                    if show_credentials:
                        ...
                        postgresql_info["credentials"] = (
                            "[SENSITIVE - Use show_credentials=True]"
                        )
            ...
            if "connection_name" in postgresql_info:
                ...
                if len(parts) == 3:
                    ...
                    postgresql_info["cloud_sql_proxy"] = (
                        f"cloud_sql_proxy -instances={postgresql_info['connection_name']}=tcp:5432"
                    )
                    postgresql_info["connection_proxy"] = (
                        f"postgresql://{username}:{password}@127.0.0.1:5432/{database}"
                    )

                    if "public_ip" in postgresql_info:
                        postgresql_info["connection_direct"] = (
                            f"postgresql://{username}:{password}@{postgresql_info['public_ip']}:5432/{database}"
                        )
    ...
    def show_status(self) -> None:
        ...
        status_data = [
            ...
            [
                "Cloud Provider",
                f"{self.provider.get('name', 'unknown')} ({self.provider.get('project_id', 'unknown')})",
            ],
    ...
    def update_teardown_schedule(
        self, duration_hours: int, time_zone: str = "UTC"
    ) -> Dict[str, Any]:
        """
    ...
    def show_teardown_status(self):
        ...
        if schedule_time:
            print(
                f"⏰ Next Execution: {schedule_time.strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
            ...
            if now < schedule_time:
                ...
                print(
                    f"   ⚠️  Scheduled time passed {hours_passed}h {minutes_passed}m ago"
                )

        last_attempt = status.get("last_attempt_time")
        if last_attempt:
            print(
                f"🕐 Last Execution: {last_attempt.strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
        else:
