...
def deploy(config_path: str, show_progress: bool = True) -> DeploymentStack:
        ...
        if not config_file.is_absolute():
            ...
            if demo_path.exists():
                ...
                if example_path.exists():
                    ...
                    print(
                        f"ℹ️  Found config file in example/config/ directory: {config_file}"
                    )
                else:
    ...
    print(
        f"☁️  Provider: {config.get('provider', {}).get('name', 'unknown')} ({config.get('provider', {}).get('project_id', 'unknown')})"
    )
...
def load(stack_name: str) -> DeploymentStack:
    ...
    if not workspace_dir.exists():
        raise FileNotFoundError(
            f"Stack '{stack_name}' not found. Deploy it first with deployml.deploy()"
        )

    # Try to find the original config file
    config_files = list(workspace_dir.glob("*.yaml")) + list(
        workspace_dir.glob("*.yml")
    )
...
def _format_deployment_line(line: str) -> str:
    ...
    if line.startswith("📦 DeployML Outputs:"):
        ...
        or line.startswith("  Monthly Cost:")
    ):
        return f"  {line}"
        ...
        "terraform" in line.lower()
        or "applying" in line.lower()
        or "creating" in line.lower()
    ):
    ...
    if process.returncode != 0:
        ...
        error_lines = [
            ...
            if any(
                ...
                for keyword in ["error", "failed", "fatal", "exception", "traceback"]
            )
        ]
        if error_lines:
