...
def upload_terraform_files_to_gcs(
    terraform_dir: Path, project_id: str, workspace_name: str
):
    """
    ...
    try:
        ...
        if not bucket_resource:
            typer.echo(
                "WARNING: Teardown module bucket not found in state. Skipping upload."
            )
            return
        ...
        terraform_files = list(terraform_dir.glob("*.tf")) + list(
            terraform_dir.glob("*.tfvars")
        )
        terraform_files += list(
            terraform_dir.glob("terraform.tfstate*")
        )  # Include state files
        terraform_files += (
            list((terraform_dir / "modules").rglob("*.tf"))
            if (terraform_dir / "modules").exists()
            else []
        )
        ...
        typer.echo(
            f" Uploaded {uploaded_count} Terraform files to gs://{bucket_name}/{workspace_name}/terraform/"
        )
...
def extract_resource_manifest(
    terraform_dir: Path, project_id: str, workspace_name: str, region: str
) -> dict:
    """
    ...
    if state_proc.returncode == 0:
        state_resources = [
            r.strip() for r in state_proc.stdout.strip().split("\n") if r.strip()
        ]
        ...
        for resource in state_resources:
            try:
                ...
                if (
                    "google_cloud_run_service" in resource
                    and "google_cloud_run_v2_job" not in resource
                ):
                    show_proc = run_tool(
                        "terraform",
                        ["state", "show", resource],
                    ...
                    if show_proc.returncode == 0:
                        ...
                        for line in show_proc.stdout.split("\n"):
                            ...
                            # Check for location or region field
                                ...
                                clean_line.strip().startswith("location")
                                or clean_line.strip().startswith("region")
                            ) and "=" in clean_line:
                                parts = clean_line.split("=")
                                if len(parts) >= 2:
                                    service_region = (
                                    ...
                                    service_region = re.sub(
                                        r"\x1b\[[0-9;]*m", "", service_region
                                    )
                        if service_name:
                            manifest["resources"]["cloud_run_services"].append(
                                {"name": service_name, "region": service_region}
                            )
                    ...
                    if show_proc.returncode == 0:
                        ...
                        if job_name:
                            manifest["resources"]["cloud_scheduler_jobs"].append(
                                {"name": job_name, "region": job_region}
                            )
...
def upload_resource_manifest(
    manifest: dict, terraform_dir: Path, project_id: str, workspace_name: str
):
    ...
    try:
        ...
        typer.echo(
            f" Resource manifest uploaded to gs://{bucket_name}/{workspace_name}/resource-manifest.json"
        )
...
def _validate_deploy_config_or_exit(config: dict) -> None:
    ...
    if not isinstance(provider, dict):
        typer.secho(
            " Config is missing required field: provider (mapping).",
            fg=typer.colors.RED,
        )
        raise typer.Exit(code=1)
    ...
    if not isinstance(deployment, dict) or not deployment.get("type"):
        typer.secho(
            " Config is missing required field: deployment.type.", fg=typer.colors.RED
        )
        raise typer.Exit(code=1)
...
def doctor(
    ...
    if terraform_installed:
        ...
        if tf_ver and tf_ver[0] >= 1:
            typer.secho(
                f"\n Terraform {'.'.join(map(str, tf_ver))} (>= 1.0)",
                fg=typer.colors.GREEN,
            )
        elif tf_ver:
            typer.secho(
                f"\n Terraform {'.'.join(map(str, tf_ver))} found, but requires >= 1.0",
                fg=typer.colors.RED,
            )
        else:
            typer.secho(
        ...
        if project_id:  # Check if not empty after stripping
            ...
            if not missing_roles:
                ...
                for r in missing_roles:
                    typer.echo(f"  - {r}")
                typer.echo(
                    "   Fix: grant roles/owner OR each role via gcloud projects add-iam-policy-binding"
                )
    elif gcp_installed:
        typer.secho(
...
def deploy(
    ...
    generate_only: bool = typer.Option(
        ...
        "--generate-only",
        "-g",
        help="Only generate manifests, do not apply (for GKE deployments)",
    ),
    verbose: bool = typer.Option(
        False,
        "--verbose",
        "-v",
        help="Stream Terraform logs to stdout instead of showing progress bar",
    ),
):
    """
    ...
    if cloud == "gcp":
        if project_marker.exists():
            ...
            if previous_project and previous_project != project_id:
                ...
                typer.echo(
                    "  Run `deployml destroy` first to clean up the old project,"
                )
                typer.echo(
                    "  or change `name:` in config.yaml to use a fresh workspace."
                )
                raise typer.Exit(code=1)
    ...
    if cloud == "gcp" and not validate_gcp_region(region, project_id):
        typer.secho(
            f" Region '{region}' is not valid for GCP. Run: gcloud compute regions list",
            fg=typer.colors.RED,
        )
        raise typer.Exit(code=1)
    ...
    if deployment_type == "gke":
        ...
        if not generate_only and not connect_to_gke_cluster(
            project_id, cluster_name, zone, region_gke
        ):
            raise typer.Exit(code=1)
        ...
        for stage in stack:
            for stage_name, tool in stage.items():
                if stage_name == "experiment_tracking" and tool.get("name") == "mlflow":
                    ...
                    image = params.get(
                        "image", f"gcr.io/{project_id}/mlflow/mlflow:latest"
                    )
                    ...
                    params = tool.get("params", {})
                    image = params.get(
                        "image", f"gcr.io/{project_id}/fastapi/fastapi:latest"
                    )
                    mlflow_uri = params.get(
    ...
    for stage in stack:
        for stage_name, tool in stage.items():
            if tool.get("params", {}).get("artifact_bucket"):
                ...
                create_bucket = tool["params"].get("create_artifact_bucket", True)
        ...
        for stage_name, tool in stage.items():
            ...
            if stage_name == "workflow_orchestration" and tool_name == "cron":
                for job in params.get("jobs", []):
                    if not job.get("image") or job.get("image", "").startswith(
                        "gcr.io/"
                    ):
    ...
    if teardown_enabled:
        ...
        teardown_cron_schedule = calculate_cron_from_timestamp(
            teardown_scheduled_timestamp
        )
    ...
    warning_threshold = cost_config.get("warning_threshold", 100.0)  # Default: $100
    ...
    if cost_enabled:
        ...
        if usage_file is None:
            try:
                ...
                cloudsql_amount = cost_config.get("cloudSQL_amount") or cost_config.get(
                    "cloudsql_amount"
                )
                bigquery_amount = cost_config.get("bigQuery_amount") or cost_config.get(
                    "bigquery_amount"
                )
                ...
                if cloudsql_amount is not None:
                    resource_type_default_usage["google_sql_database_instance"] = {
        ...
        if result_code == 0:
            ...
            if teardown_enabled:
                try:
                    ...
                    upload_terraform_files_to_gcs(
                        DEPLOYML_TERRAFORM_DIR, project_id, workspace_name
                    )

                    # Extract and upload resource manifest
                    typer.echo(" Extracting resource manifest...")
                    manifest = extract_resource_manifest(
                        DEPLOYML_TERRAFORM_DIR, project_id, workspace_name, region
                    )
                    upload_resource_manifest(
                        manifest, DEPLOYML_TERRAFORM_DIR, project_id, workspace_name
                    )
                    typer.echo(" Resource manifest uploaded successfully")

                except Exception as e:
                    typer.echo(
                        f"WARNING: Warning: Could not upload files/manifest to GCS: {e}"
                    )
                    typer.echo(
                        "   Teardown may not work automatically. Manual teardown required."
                    )
            ...
            if teardown_enabled:
                ...
                teardown_scheduled_timestamp = int(teardown_at.timestamp())
                correct_cron_schedule = calculate_cron_from_timestamp(
                    teardown_scheduled_timestamp
                )
                ...
                try:
                    typer.echo(
                        f" Updating teardown schedule to: {teardown_at.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                    )
                ...
                typer.echo(
                    f"   To cancel: deployml teardown cancel --config-path {config_path}"
                )
            ...
            typer.secho(
                f"\n Terraform apply failed with exit code {result_code}",
                fg=typer.colors.RED,
                bold=True,
            )
...
def get_urls(
    ...
    show_secrets: bool = typer.Option(
        False,
        "--show-secrets",
        help="Fetch and print Grafana admin password and MLflow DSN connection hint. Uses gcloud secrets versions access.",
    ),
):
    """
    ...
    if not terraform_dir.exists():
        typer.echo(
            f" No deployment found at {terraform_dir}. Run 'deployml deploy' first."
        )
        raise typer.Exit(code=1)
    ...
    for key, value in outputs.items():
        ...
        if is_sensitive:
            ...
            if output_val.startswith("http://") or output_val.startswith("https://"):
                typer.secho(
                    f"  {key}: {output_val}", fg=typer.colors.BRIGHT_BLUE, bold=True
                )
            ...
            for subkey, subval in output_val.items():
                ...
                if isinstance(subval, str) and (
                    subval.startswith("http://") or subval.startswith("https://")
                ):
                    typer.secho(
                        f"    {subkey}: {subval}",
                        fg=typer.colors.BRIGHT_BLUE,
    ...
    if show_secrets:
        ...
        grafana_secret = outputs.get("grafana_admin_password_secret_id", {}).get(
            "value", ""
        )
        if grafana_secret and project_id:
            ...
            if fetch.returncode == 0:
                ...
                typer.echo(
                    f"  grafana_admin_password: (fetch failed: {fetch.stderr.strip()})"
                )
        ...
        if instance and dsn_secret and project_id:
            typer.echo("")
            typer.echo(
                "  To connect to MLflow Postgres from your laptop, run the Cloud SQL Auth Proxy:"
            )
            ...
            typer.echo(
                f"    gcloud secrets versions access latest --secret={dsn_secret} --project={project_id}"
            )
...
def destroy(
    ...
    try:
        ...
        if state_result.returncode == 0:
            resources_to_remove = [
                r.strip()
                for r in state_result.stdout.splitlines()
                if any(
                    x in r
                    for x in [
                        "google_sql_database.",
        ...
        if result.returncode == 0:
            ...
            if keep_images:
                ...
                run_tool(
                    "gcloud",
                    ["storage", "rm", "--recursive", cb_bucket, "--quiet"],
                    capture_output=True,
                )
            else:
                typer.echo(
                    f" Keeping Artifact Registry repo {repository} and {cb_bucket}."
                )
...
def status(
    ...
    if not tf_dir.exists():
        typer.secho(
            "Status: not deployed (no terraform workspace found)",
            fg=typer.colors.YELLOW,
        )
    ...
    if out_proc.returncode == 0 and out_proc.stdout.strip():
        try:
            ...
            urls = {
                ...
                and v.get("value", "").startswith("http")
            }
            if urls:
                ...
                typer.secho(
                    "Status: workspace exists but no URL outputs found",
                    fg=typer.colors.YELLOW,
                )
        except Exception:
            typer.secho(
                "Status: workspace exists but terraform output is not parseable",
                fg=typer.colors.YELLOW,
            )
    else:
        typer.secho(
            "Status: workspace exists but terraform output is empty",
            fg=typer.colors.YELLOW,
        )
...
def teardown(
    action: str = typer.Argument(
        ..., help="Action: cancel, status, update, or schedule"
    ),
    config_path: Path = typer.Option(
    ...
    if action == "cancel":
        ...
        typer.echo(
            f" Unknown action: {action}. Use: cancel, status, update, or schedule"
        )
        raise typer.Exit(code=1)
...
def cancel_teardown(config: dict, deployml_dir: Path, workspace_name: str):
    ...
    if result.returncode == 0:
        ...
        typer.echo(
            "   The scheduler job may not exist or may have already been deleted."
        )
...
def show_teardown_status(config: dict, deployml_dir: Path, workspace_name: str):
    ...
    if result.returncode != 0:
        ...
        typer.echo(
            "   Teardown may not be scheduled or may have already been cancelled."
        )
...
def update_teardown_schedule(
    config: dict, deployml_dir: Path, workspace_name: str, duration_hours: int = 24
):
    ...
    try:
        ...
        if schedule_time:
            ...
            typer.echo(
                f" Current Schedule: {current_time.strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
    except Exception:
        pass
    ...
    if verify_result.returncode == 0:
        try:
            ...
            if updated_schedule_time:
                ...
                if actual_time.strftime("%Y-%m-%d %H:%M") != teardown_at.strftime(
                    "%Y-%m-%d %H:%M"
                ):
                    typer.echo(
...
def schedule_teardown(
    ...
    typer.echo(
        f" Teardown scheduled for: {teardown_at.strftime('%Y-%m-%d %H:%M:%S UTC')}"
    )
    typer.echo(
        "WARNING:Note: This only updates local metadata. To actually schedule teardown,"
    )
...
def init(
    ...
    if provider == "gcp":
        ...
        if not check_gcp_adc():
            typer.secho(
                " Application Default Credentials missing. Run: gcloud auth application-default login",
                fg=typer.colors.RED,
            )
            raise typer.Exit(code=1)
        if not validate_gcp_project(project_id):
            typer.secho(
                f" Project '{project_id}' not found or not accessible by your gcloud account.",
                fg=typer.colors.RED,
            )
                ...
                "stack": [
                    {
                        "experiment_tracking": {
                            "name": "mlflow",
                            "params": {"service_name": "mlflow-server"},
                        }
                    },
                    {
                        "artifact_tracking": {
                            "name": "mlflow",
                            "params": {
                                "artifact_bucket": f"mlflow-artifacts-{project_id}"
                            },
                        }
                    },
                    {
                        "model_registry": {
                            "name": "mlflow",
                            "params": {"backend_store_uri": "postgresql"},
                        }
                    },
                    {
                        "model_serving": {
                            "name": "fastapi",
                            "params": {"service_name": "fastapi-mlflow-server"},
                        }
                    },
                    {
                        "model_monitoring": {
...
def minikube_init(
    ...
    generate_fastapi_manifests(
        output_dir=output_dir, image=image, mlflow_tracking_uri=mlflow_uri
    )

    typer.echo("\nSetup complete! Next steps:")
    typer.echo(f"  1. Edit the manifests in {output_dir} if needed")
    typer.echo(
        f"  2. Deploy with: deployml minikube-deploy --manifest-dir {output_dir}"
    )
...
def minikube_deploy(
    ...
    success = deploy_fastapi_to_minikube(
        manifest_dir, image_name=image_name, namespace=namespace
    )
...
def mlflow_init(
    ...
    image: str = typer.Option(..., "--image", "-i", help="MLflow Docker image"),
    backend_store_uri: Optional[str] = typer.Option(
        None,
        "--backend-store-uri",
        "-b",
        help="Backend store URI. Default sqlite on the mounted PVC.",
    ),
    artifact_root: Optional[str] = typer.Option(
        None,
        "--artifact-root",
        "-a",
        help="Artifact root path (defaults to /mlflow-artifacts)",
    ),
    start_cluster: bool = typer.Option(
        True,
        "--start-cluster/--no-start-cluster",
...
def mlflow_deploy(
    ...
    success = deploy_mlflow_to_minikube(
        manifest_dir, image_name=image_name, namespace=namespace
    )
...
def gke_deploy(
    ...
    zone: Optional[str] = typer.Option(None, "--zone", "-z", help="GKE cluster zone"),
...
def gke_cluster_create(
    ...
    region: str = typer.Option(
        "us-west1", "--region", "-r", help="Region for the cluster"
    ),
    ...
    typer.echo(
        f" Creating {'Autopilot' if autopilot else 'standard'} cluster {cluster}..."
    )
    ...
    typer.echo(
        f"   Next: deployml gke-init --output-dir manifests --image gcr.io/{project}/... --project {project}"
    )
...
def gke_destroy(
    ...
    for fname in ["service.yaml", "deployment.yaml", "pvc.yaml"]:
        ...
        if f.exists():
            ...
            result = run_tool(
                "kubectl",
                ["delete", "-f", str(f), "--ignore-not-found"] + ns,
                capture_output=True,
                text=True,
            )
            if result.returncode == 0:
    ...
    if not keep_images:
        ...
        if dep.exists():
            try:
                ...
                image = doc["spec"]["template"]["spec"]["containers"][0].get(
                    "image", ""
                )
            except Exception:
                image = ""
...
def gke_init(
    ...
    mlflow_uri: Optional[str] = typer.Option(
        ...
        "--mlflow-uri",
        "-m",
        help="MLflow URI (for FastAPI). Only used when service is fastapi.",
    ),
    mlflow_image: Optional[str] = typer.Option(
        None,
        "--mlflow-image",
        help="MLflow image. Required when --service all. Defaults to --image when not provided.",
    ),
):
    """
    ...
    if service == "mlflow":
        ...
        typer.echo("\nNext steps:")
        typer.echo(
            f"  1. deployml gke-deploy -d {ml_dir} -c CLUSTER -p {project} -r REGION"
        )
        typer.echo(
            f"  2. deployml gke-deploy -d {fa_dir} -c CLUSTER -p {project} -r REGION"
        )
    else:
...
def gke_apply(
    ...
    if deployment_type != "gke":
        typer.echo(
            f"This command is only for GKE deployments. Found: {deployment_type}"
        )
        raise typer.Exit(code=1)
    ...
    if not manifests_dir.exists():
        ...
        typer.echo(
            "   Generate manifests first with: deployml deploy --config-path <config> --generate-only"
        )
        raise typer.Exit(code=1)
