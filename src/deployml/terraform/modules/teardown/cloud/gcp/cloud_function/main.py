...
def teardown_infrastructure(request):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if (terraform_dir / "main.tf").exists():
                ...
                if init_result.returncode != 0:
                    ...
                    return destroy_via_remote_state(
                        project_id, workspace_name, terraform_state_bucket
                    )
                ...
                if destroy_result.returncode == 0:
                    logger.info(
                        f"Successfully destroyed infrastructure for {workspace_name}"
                    )
                    return {
                        "status": "success",
                ...
                logger.info(
                    "No terraform files found, attempting alternative teardown method"
                )
                return destroy_via_remote_state(
                    project_id, workspace_name, terraform_state_bucket
                )
        ...
        return {"status": "error", "message": "Teardown operation timed out"}, 500
    except Exception as e:
...
def download_terraform_files(
    bucket_name: str, workspace_name: str, terraform_dir: Path
):
