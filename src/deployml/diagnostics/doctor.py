...
class DeployMLDoctor:
    ...
    def _check_docker(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self._add_result(
                    CheckResult(name="Docker", status=CheckStatus.PASS, message=version)
                )
            else:
                self._add_result(
                    CheckResult(
    ...
    def _check_terraform(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self._add_result(
                    CheckResult(
                        name="Terraform", status=CheckStatus.PASS, message=version_line
                    )
                )
            else:
                self._add_result(
    ...
    def _check_infracost(self):
        ...
        try:
            result = run_tool(
                "infracost", ["--version"], capture_output=True, text=True
            )
            if result.returncode == 0:
                version = result.stdout.strip()
                self._add_result(
    ...
    def _print_simple_table(self, show_all: bool):
        ...
        for result in self.results:
            if (
                not show_all
                and result.status == CheckStatus.INFO
            ...
            print(
                f"{status_text:<{max_status_width}} {result.name:<{max_name_width}} {result.message}"
            )

            if result.fix_command:
                print(
                    f"{'':<{max_status_width}} {'':<{max_name_width}} Fix: {result.fix_command}"
                )
