...
def test_run_tool_forwards_resolved_path_args_and_kwargs():
    ...
    with (
        patch.object(pc, "resolve_tool", return_value="/usr/local/bin/gcloud"),
        patch.object(pc.subprocess, "run", return_value=completed) as mock_run,
    ):
        result = pc.run_tool(
            "gcloud", ["version", "--quiet"], capture_output=True, check=True
        )
...
def test_configure_console_encoding_forces_utf8_on_windows():
    ...
    fake_sys.stdout.reconfigure.assert_called_once_with(
        encoding="utf-8", errors="replace"
    )
    fake_sys.stderr.reconfigure.assert_called_once_with(
        encoding="utf-8", errors="replace"
    )
